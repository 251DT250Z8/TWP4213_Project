...
switch($action) {
    case 'getAll':
        $data = [
            "users" => $pdo->query("SELECT id, email, role, firstName, lastName, phoneExt, phone, addr1, city, state, zip, country FROM users")->fetchAll(PDO::FETCH_ASSOC),
            "categories" => $pdo->query("SELECT * FROM categories")->fetchAll(PDO::FETCH_ASSOC),
            "products" => $pdo->query("SELECT * FROM products")->fetchAll(PDO::FETCH_ASSOC),
            "orders" => array_map(function($o) { $o['items'] = json_decode($o['items']); return $o; }, $pdo->query("SELECT * FROM orders")->fetchAll(PDO::FETCH_ASSOC)),
